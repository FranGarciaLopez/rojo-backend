...
async getData(req, res) {
    try {
        ...
        // Find questionnaires where `id` matches the provided id
            const userObjectId = new mongoose.Types.ObjectId(id);            
        const userEntries = await Questionnaire.find({ id: userObjectId });
        if (userEntries.length === 0) {
            return res.status(404).json({ message: 'No questionnaires found for this user' });
        }
        userEntries.forEach(entry => {
            entry.TextQuestions = get_text_questions_formOBJ_in_str(DataQuestionnaire,';');
            entry.TextAnswers   = get_text_answers_in_str(DataQuestionnaire,';','|');
        });
